...
<body>
  <nav class="navbar navbar-expand-lg navbar-light ">
    <div class="container-fluid">
      ...
      <a class="navbar-brand" href="index.php">Ticketster</a>
      ...
      <div class="collapse navbar-collapse" id="navbarNav">
        ...
        <a class="nav-link" href="index.php">Home</a>
          ...
          <li class="nav-item">
          <a class="nav-link active" aria-current="page" href= <?php echo "'".$v.".html'" ?> ><?php echo $v ?></a>
          </li>
          <?php 
  ...
  <?php
    if($con){
        $city = $_POST['city'];
        $places = mysqli_query($con,"SELECT MH_NAME,IMG_LINK,INDIAN FROM `mh_table` WHERE MH_LOCATION ='$city';");
        while ($row = mysqli_fetch_assoc($places)) {
            echo "
                <form action='detail2.php' method='post'>
                <input type='hidden' value='".$row["INDIAN"]."' name='amt'>
                <input type='hidden' value='".$row["MH_NAME"]."' name='des'>
                <button type='submit' style='background-color: rgba(240, 248, 255, 0);border: none;'>
                    ...
                    <h3 align='center' id='name' style='margin-top:80%;'>".$row["MH_NAME"]."</h3>
                    </div>
                </button>
                </form>
            " ;
